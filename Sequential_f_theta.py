...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation=F.relu,
        ...
        self.self_attn2 = MultiheadAttention(n_head=nhead, d_model=d_model, d_head=d_model, dropout=dropout, dropatt=0, 
                 pre_lnorm=norm_first)
        ...
        self.linear12 = nn.Linear(d_model, d_model, **factory_kwargs)
        self.dropout2 = nn.Dropout(dropout)
        self.linear22 = nn.Linear(d_model, d_model, **factory_kwargs)
    ...
    def forward(self, src: Tensor, deltat: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        x = src
        x  = self.norm1(x.to(device) + (deltat[None,:,None]).to(device) * self.self_attn(x.to(device), attn_mask=src_mask.to(device))) # This is to capture the time scheme
        x = self.linear2(self.dropout(self.activation(self.linear1(x))))
        x  = self.norm2(x.to(device) + (deltat[None,:,None]).to(device) * self.self_attn2(x.to(device), attn_mask=src_mask.to(device))) # This is to capture the time scheme
        x = self.linear22(self.dropout2(self.activation(self.linear12(x))))
...
file_name = 'Sequential_linear.png'
